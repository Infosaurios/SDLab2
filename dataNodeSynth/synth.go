...
func main() {
	...
	go func() {
		listener, err := net.Listen("tcp", portDataNodeSynth) //conexion sincrona escucha
		if err != nil {
			panic("La conexion con dataNodeSynth no se pudo crear" + err.Error())
		}
		grpcServer := grpc.NewServer()
		pb.RegisterMessageServiceServer(grpcServer, &server{})
		if err = grpcServer.Serve(listener); err != nil {
			panic("El servidor dataNodeSynth no se pudo iniciar" + err.Error())
		}
	}()
