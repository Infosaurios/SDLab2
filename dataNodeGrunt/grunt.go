...
func writeInDataFile(tipo_ string, id_ string, data_ string) {
	f, err := os.OpenFile("dataNodeGrunt/DATA.txt", os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func dataById(id string) string {
	idData := ""
	f, err := os.Open("dataNodeGrunt/DATA.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
...
func main() {
	...
	go func() {
		listener, err := net.Listen("tcp", portDataNodeGrunt) //conexion sincrona escucha
		if err != nil {
			panic("La conexion con dataNodeGrunt no se pudo crear" + err.Error())
		}
		grpcServer := grpc.NewServer()
		pb.RegisterMessageServiceServer(grpcServer, &server{})
		if err = grpcServer.Serve(listener); err != nil {
			panic("El servidor dataNodeGrunt no se pudo iniciar" + err.Error())
		}
	}()
